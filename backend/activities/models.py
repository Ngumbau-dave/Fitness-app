...
# MET values for calorie calculation (standard estimates)
MET_VALUES = {
    'RUN': 10.0,
    'CYC': 8.0,
    'SWM': 10.0,
...
class Activity(models.Model):
    ...
    duration = models.PositiveIntegerField(help_text='Duration in minutes')  # Fixed: removed invalid [int]
    ...
    def save(self, *args, **kwargs):
        if self.duration:
            ...
            weight_kg = 70  # Default weight — can be customized per user later
...
# Save profile when User is saved (e.g., when updating user details)
@receiver(post_save, sender=User)
